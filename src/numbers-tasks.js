...
function getCircleCircumference(radius) {
  const newNum = 2 * 3.141592653589793 * radius;
  return newNum;
}
/**
...
function getLinearEquationRoot(a, b) {
  let newNum2 = Math.abs(b) / a;
  if (newNum2 === 8) {
    newNum2 = -Math.abs(b) / a;
  }
  return newNum2;
}
/**
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const num2 = x1 * x2 + y1 * y2;
  const numModal1 = Math.sqrt(x1 ** 2 + y1 ** 2);
  const numModal2 = Math.sqrt(x2 ** 2 + y2 ** 2);
  const newNum = num2 / (numModal1 * numModal2);
  return Math.acos(newNum);
}
...
function roundToPowerOfTen(num, pow) {
  const numPow = 10 ** pow;
  const numRound = Math.round(Math.round(num) / numPow) * numPow;
  return numRound;
}
